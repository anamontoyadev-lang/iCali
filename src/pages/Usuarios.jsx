...
const ROLES = [
  ...
  { value:'asesor_call_center', label:'Asesor Call Center',   color:'#6b7280' },
  { value:'garantias',          label:'Garantías',            color:'#ec4899' },
  { value:'laboratorio',        label:'Laboratorio',          color:'#f97316' },
  { value:'inventario',         label:'Inventario',           color:'#06b6d4' },
...
export default function Usuarios() {
  ...
  const [formCrear, setFormCrear] = useState({ email:'', password:'', nombre:'', apellido:'', rol:'asesor_mostrador' })
  ...
  async function loadUsuarios() {
    const { data } = await supabase.from('perfiles').select('id,nombre,apellido,email,rol,fecha_ingreso,sede').order('rol')
    setUsuarios(data || [])
    setLoading(false)
  }
  ...
  async function crearUsuario(e) {
    e.preventDefault()
    setSaving(true); setMsgError('')
    try {
      const authUser = await adminFetch('/users', 'POST', { email: formCrear.email, password: formCrear.password, email_confirm: true })
      const { error } = await supabase.from('perfiles').insert({ id: authUser.id, nombre: formCrear.nombre, apellido: formCrear.apellido, email: formCrear.email, rol: formCrear.rol })
      if (error) throw new Error(error.message)
      setMsgExito(`Usuario ${formCrear.email} creado`)
      setModalCrear(false)
  ...
  async function editarUsuario(e) {
    ...
    setSaving(true); setMsgError('')
    try {
      const { error } = await supabase.from('perfiles').update({ nombre: formEditar.nombre, apellido: formEditar.apellido, rol: formEditar.rol }).eq('id', modalEditar.id)
      if (error) throw new Error(error.message)
  ...
  async function cambiarPassword(e) {
    ...
    try {
      ...
      setModalPass(null); setNuevaPass(''); setConfirmaPass('')
    } catch (err) { setMsgError(err.message) }
    setSaving(false)
    setTimeout(() => { setMsgExito(''); setMsgError('') }, 4000)
  }
  ...
  const grupos = ROLES.map(r => ({ ...r, usuarios: filtrados.filter(u => u.rol === r.value) })).filter(g => g.usuarios.length > 0)

  if (!esAdmin) return <div style={{ padding:40, color:'#4a6a8a', textAlign:'center', fontSize:14, fontFamily:"'DM Sans', system-ui" }}>No tienes permisos para acceder a este módulo.</div>
  ...
  return (
    <div style={{ padding:'32px 36px', fontFamily:"'DM Sans', system-ui" }}>
      ...
      {msgError && <div style={{ marginBottom:16, padding:'10px 16px', background:'rgba(244,63,94,0.1)', border:'1px solid rgba(244,63,94,0.3)', borderRadius:8, color:'#f87171', fontSize:13 }}>⚠ {msgError}</div>}
      ...
      {loading ? <div style={{ color:'#4a6a8a', fontSize:13 }}>Cargando usuarios...</div>
      : grupos.length === 0 ? <div style={{ color:'#4a6a8a', fontSize:13 }}>No se encontraron usuarios.</div>
      : grupos.map(grupo => (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {grupo.usuarios.map(u => (
              <div key={u.id} style={{ background:'#0d1a35', border:'1px solid #1a2f52', borderRadius:10, padding:'14px 18px', display:'flex', alignItems:'center', gap:14 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ color:'#fff', fontSize:14, fontWeight:500 }}>{u.nombre} {u.apellido}</div>
                  <div style={{ color:'#4a6a8a', fontSize:12, marginTop:2 }}>{u.email}</div>
                </div>
                <RolBadge rol={u.rol} />
                <div style={{ display:'flex', gap:6 }}>
                  <button onClick={() => { setModalEditar(u); setFormEditar({ nombre:u.nombre||'', apellido:u.apellido||'', rol:u.rol }); setMsgError('') }}
                    style={{ background:'#1a2f52', border:'none', borderRadius:6, color:'#8aabcc', fontSize:12, padding:'6px 12px', cursor:'pointer' }}>✏️ Editar</button>
                  <button onClick={() => { setModalPass({ id:u.id, email:u.email }); setMsgError('') }}
                    style={{ background:'#1a2f52', border:'none', borderRadius:6, color:'#8aabcc', fontSize:12, padding:'6px 12px', cursor:'pointer' }}>🔑 Contraseña</button>
                  <button onClick={() => eliminarUsuario(u.id, u.email)}
                    style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:6, color:'#ef4444', fontSize:12, padding:'6px 12px', cursor:'pointer' }}>🗑️</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <Modal titulo="Crear nuevo usuario" onClose={() => setModalCrear(false)}>
          <form onSubmit={crearUsuario}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px 14px' }}>
              <FormField label="Nombre *"><input required style={inp} value={formCrear.nombre} onChange={e => setFormCrear(f=>({...f,nombre:e.target.value}))} /></FormField>
              <FormField label="Apellido *"><input required style={inp} value={formCrear.apellido} onChange={e => setFormCrear(f=>({...f,apellido:e.target.value}))} /></FormField>
              <FormField label="Correo electrónico *" span={2}><input required type="email" style={inp} value={formCrear.email} onChange={e => setFormCrear(f=>({...f,email:e.target.value}))} placeholder="nombre@icali.co" /></FormField>
              <FormField label="Contraseña *" span={2}><input required type="password" style={inp} value={formCrear.password} onChange={e => setFormCrear(f=>({...f,password:e.target.value}))} placeholder="Mínimo 8 caracteres" minLength={8} /></FormField>
      ...
      {modalEditar && (
        <Modal titulo={`Editar: ${modalEditar.nombre} ${modalEditar.apellido}`} onClose={() => setModalEditar(null)}>
          <form onSubmit={editarUsuario}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px 14px' }}>
              <FormField label="Nombre *"><input required style={inp} value={formEditar.nombre} onChange={e => setFormEditar(f=>({...f,nombre:e.target.value}))} /></FormField>
              <FormField label="Apellido *"><input required style={inp} value={formEditar.apellido} onChange={e => setFormEditar(f=>({...f,apellido:e.target.value}))} /></FormField>
              <FormField label="Rol *" span={2}>
      ...
      {modalPass && (
        <Modal titulo="Cambiar contraseña" onClose={() => { setModalPass(null); setNuevaPass(''); setConfirmaPass('') }}>
          <form onSubmit={cambiarPassword}>
            <div style={{ marginBottom:16, padding:'10px 14px', background:'rgba(59,130,246,0.08)', border:'1px solid rgba(59,130,246,0.2)', borderRadius:8, color:'#8aabcc', fontSize:12 }}>📧 {modalPass.email}</div>
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <FormField label="Nueva contraseña *"><input required type="password" style={inp} value={nuevaPass} onChange={e => setNuevaPass(e.target.value)} placeholder="Mínimo 8 caracteres" minLength={8} /></FormField>
              <FormField label="Confirmar contraseña *"><input required type="password" style={inp} value={confirmaPass} onChange={e => setConfirmaPass(e.target.value)} placeholder="Repite la contraseña" /></FormField>
            </div>
            {msgError && <ErrMsg msg={msgError} />}
            <BotonesModal onCancel={() => { setModalPass(null); setNuevaPass(''); setConfirmaPass('') }} saving={saving} labelOk="Cambiar contraseña" />
          </form>
        </Modal>
      )}
    </div>
...
function FormField({ label, children, span }) {
  return (
    <div style={{ gridColumn: span ? `span ${span}` : undefined }}>
      <label style={{ display:'block', color:'#8aabcc', fontSize:11, fontWeight:500, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:5 }}>{label}</label>
      {children}
    </div>
  )
}
...
function ErrMsg({ msg }) {
  return <div style={{ marginTop:12, padding:'10px 14px', background:'rgba(244,63,94,0.1)', border:'1px solid rgba(244,63,94,0.3)', borderRadius:8, color:'#f87171', fontSize:13 }}>⚠ {msg}</div>
}
